...
- Current paper-aligned Ohm stochastic baseline outputs are fixed for reproducibility: atom contact cutoff 3.4 A, alpha 3.0, random seed 20260525, and 10,000 rounds for primary ACI/pathway runs. Old smoke, sensitivity, and legacy approximation run artifacts have been removed from the active artifact set. This now covers the challenge-facing Ohm preprocessing, ACI propagation, selected allosteric-site ranking, hotspot clustering, pathway histogram, Eq. 4 critical-residue scoring, pairwise-correlation artifact generation, and validation-only evaluation.
...
- Core Ohm propagation now uses Wang et al. Eq. 3: `P_ij = 1 - exp(-alpha * C_ij / atom_count_i)`, alpha 3.0, and separate `V`/`W` traversal semantics. Hotspot clustering uses 4.5 A minimum atom-distance neighbors, excludes active-site seeds for allosteric hotspot reporting, and assigns residues toward higher-ACI neighbors. Only the current `ohm_atom_contacts_strict_primary_*` outputs are retained.
- 2026-05-27 Ohm full-reproduction verification status: challenge-target reproduction delivered for KRAS/BCR-ABL1; current code is `challenge_ohm_baseline_delivered__paper_benchmark_external_validation_open`. Remaining gap to a publication-level Wang et al. reproduction is external paper-benchmark replication against Amor/Caspase-1/CheY/CHESCA data and official Bitbucket source cross-check if access becomes available. See `docs/agent-harness/reviews/ohm-full-reproduction-verification-2026-05-27.zh-TW.md`.
- Current Ohm challenge scoring reads validation labels only after prediction artifacts exist. The scorer now reports seed-label overlap, non-seed AP/AUROC, hotspot same-size random enrichment, pathway coverage, critical-residue ranking, pairwise-correlation endpoint coverage, and graph comparator baselines. KRAS primary score: residue top-10 hits 4/10, AUROC 0.66850105, non-seed AP 0.37215865, with 13/49 validation labels overlapping seed residues. BCR-ABL1 primary score: residue top-10 hits 0/10, AUROC 0.51717448, non-seed AP 0.10374819, with 0/46 validation labels overlapping seed residues.
- 2026-05-27 Ohm sensitivity suite remains available through `make ohm-suite` / `scripts/pipeline/baselines/ohm/run_ohm_baseline_suite.py`, but old sensitivity outputs and aggregate summaries have been removed so the repository artifact set reflects only the current primary OHM pipeline run.
...
| --- | --- | --- | --- |
| WP-1 | Baseline graph metrics | Delivered for Ohm challenge baseline | Core Ohm propagation, hotspot clustering, pathway/critical-residue outputs, pairwise-correlation outputs, and KRAS/BCR challenge scoring are implemented; BCR residue-ranking weakness remains a scientific result rather than a pipeline blocker. |
...
| WP-3 | Scoring harness | Delivered for Ohm challenge baseline | Current scorer emits per-run score reports, score traces, seed-label diagnostics, hotspot enrichment, pathway/critical/pairwise diagnostics, and comparator baselines for retained primary runs. |
